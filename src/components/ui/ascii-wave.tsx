...
const AsciiWave: React.FC<AsciiWaveProps> = ({
    className,
    color = "#e05e46",  // Customized to match EMEI Theme Orange
    speed = 1
}) => {
        ...
        const draw = () => {
            ...
            for (let x = 0; x < columns; x++) {
                ...
                for (let y = rows - 1; y > rows - activeRows; y--) {
                    ...
                    const colFade = Math.min(1, distFromTop / 6);

                    // Edge fade to prevent abrupt cuts at container boundaries (left/right/top/bottom)
                    const FADE_MARGIN_X = 120;
                    const FADE_MARGIN_Y = 80;
                    
                    const edgeFadeX = Math.min(1, Math.min(posX, width - posX) / FADE_MARGIN_X);
                    const edgeFadeY = Math.min(1, Math.min(posY, height - posY) / FADE_MARGIN_Y);
                    const edgeFade = Math.max(0, edgeFadeX * edgeFadeY);

                    const fade = colFade * edgeFade;
