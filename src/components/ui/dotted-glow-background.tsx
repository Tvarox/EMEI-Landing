...
type DottedGlowBackgroundProps = {
  ...
  speedScale?: number;
  /** if true, dots fade out smoothly towards the edges and corners */
  fadeEdge?: boolean;
};
...
export const DottedGlowBackground = ({
  ...
  darkColor = "rgba(255, 85, 0, 0.4)",
  glowColor = "rgba(255, 85, 0, 0.95)",
  darkGlowColor = "rgba(255, 85, 0, 0.95)",
  colorLightVar,
  colorDarkVar,
  glowColorLightVar,
  glowColorDarkVar,
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      for (let i = 0; i < dots.length; i++) {
        ...
        let dotOpacity = a * opacity;

        if (fadeEdge) {
          const distToLeft = d.x;
          const distToRight = width - d.x;
          const distToTop = d.y;
          const distToBottom = height - d.y;
          const minDist = Math.min(distToLeft, distToRight, distToTop, distToBottom);
          const fadeMargin = 100; // fade out in the outer 100px of borders and corners
          if (minDist < fadeMargin) {
            const ratio = Math.max(0, minDist / fadeMargin);
            dotOpacity *= ratio * ratio * (3 - 2 * ratio); // smooth Hermite interpolation
          }
        }

        // draw glow when bright and not fully faded out
        if (a > 0.6 && dotOpacity > 0.1) {
